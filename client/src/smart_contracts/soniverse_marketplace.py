...
class Marketplace(sp.Contract):
    ...
    @sp.entry_point
    def sellOffer(self, params):
        # Check that the caller is the owner of the NFT
        nft = self.data.data[params.token_id]
        sp.verify(nft.holder == sp.sender)
    
        # Update the owner and price of the NFT
        nft.holder = sp.self_address
        nft.amount = params.price

        # Set the NFT to be collectable again
        nft.collectable = True
        self.fa2_transfer(self.data.token, sp.sender, sp.self_address, params.token_id, 1)
...
def test():
    ...
    token_contract = Token(FA2.FA2_config(non_fungible = True), admin = admin.address, metadata = sp.utils.metadata_of_url("0"))
    
    scenario += token_contract
    scenario.h1("MarketPlace")
    marketplace = Marketplace(token_contract.address, sp.utils.metadata_of_url("0"), admin.address)
    scenario += marketplace
    scenario.h1("Mint")
    scenario += marketplace.mint(sp.record(amount = 200000000, metadata = sp.pack("test"))).run(sender = admin, valid = False)
    scenario += token_contract.set_administrator(marketplace.address).run(sender = admin)
    scenario += marketplace.mint(sp.record(amount = 200000000, metadata = sp.pack("test"))).run(sender = admin)
    scenario += marketplace.mint(sp.record(amount = 6000000, metadata = sp.pack("123423"))).run(sender = user1)
    scenario.h1("Collect")
    scenario += marketplace.collect(sp.record(token_id = 1)).run(sender = user2, amount = sp.mutez(6000000))
